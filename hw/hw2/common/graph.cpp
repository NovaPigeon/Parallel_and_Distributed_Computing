...
// randomize the length of graph edges in a fixed random seed
#define RANDOM_SEED 2024
#define EDGE_LEN_MAX 50
...
void build_edges(graph* graph, int* scratch)
{
    ...
    graph->outgoing_edges_len = (int*)malloc(sizeof(int) * graph->num_edges);
    for(int i = 0; i < graph->num_edges; i++)
    {
        graph->outgoing_edges[i] = scratch[num_nodes + i];
        graph->outgoing_edges_len[i] = rand()%EDGE_LEN_MAX + 1;
    }
}
...
void print_graph(const graph* graph)
{
    ...
    for (int i=0; i<graph->num_nodes; i++) {
        ...
        for (int j=start_edge; j<end_edge; j++) {
            ...
            int len = graph->outgoing_edges_len[j];
            printf("%d(len=%d) ", target, len);
        }
        printf("\n");
...
Graph load_graph_binary(const char* filename)
{
    ...
    for(int i = 0; i < graph->num_edges; i++) {
        // editted by ruiyang 
        // original graph data do not have edge weight
        // so we randomize the length of each edge
        graph->outgoing_edges_len[i] = rand()%EDGE_LEN_MAX + 1;
    }
